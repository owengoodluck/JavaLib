...
<script type="text/javascript">
	$(function() {
		$("#queryOrderDateFrom").datepicker({
			"dateFormat" : "yy-mm-dd"
		});
		$("#queryOrderDateTo").datepicker({
			"dateFormat" : "yy-mm-dd"
		});
	});
				...
				<div class="col-md-3" align="left">
					...
					>
						<option value="latestDate"
							<c:if test="${ statisticType == 'latestDate' }"> selected="selected"</c:if>
						>按销售日期排序</option>
						<option value="sellCount"
							<c:if test="${ statisticType == 'sellCount' }"> selected="selected"</c:if>
						>按销售数量排序</option>
					</select>
				</div>
			</form>
			...
			<tbody>
				<c:forEach items="${list}" var="item" varStatus="status">
					<tr align="left">
						...
						<td width="9%"><img
							src="/wms-web/img${item.getLocalImagePath()}" height="40"
							onclick='window.open("/wms-web/img${item.getLocalImagePath()}")'
						></td>
						<td align="left" width="9%"><a
							href='<c:url value="/prod/edit/${item.itemSku}" />'
							target="_blank"
						>${item.itemSku}</a></td>
						<td align="left" width="9%">${item.sellCount}</td>
						<td align="left" width="9%"><c:if
								test="${ statisticType == 'latestDate' }"
							>
